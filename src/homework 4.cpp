...
HRESULT InitWindow(HINSTANCE hInstance, int nCmdShow) {
  ...
  g_hWnd = CreateWindow(L"TutorialWindowClass", L"Hot Potato 2.0", 
...
HRESULT CompileShaderFromFile(WCHAR* szFileName, LPCSTR szEntryPoint, 
  ...
  hr = D3DX11CompileFromFile(szFileName, NULL, NULL, szEntryPoint, 
                             szShaderModel, dwShaderFlags, 0, NULL, ppBlobOut, 
                             &pErrorBlob, NULL);
  if (FAILED(hr)) {
    if (pErrorBlob != NULL) {
...
void Render() {
  ...
  g_pImmediateContext->IASetVertexBuffers(0, 1, &g_pVertexBuffer_sky, &stride,
                                          &offset);
  ...
  g_pImmediateContext->IASetVertexBuffers(0, 1, &g_pVertexBuffer, &stride, 
                                          &offset);
  level1.render_level(g_pImmediateContext, g_pVertexBuffer, &view, 
